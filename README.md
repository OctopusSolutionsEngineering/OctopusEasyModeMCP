...
| `EASY_MODE_MCP_DOWNLOAD_ARTIFACTS` | `true` | When `true`, artifacts created by the runbook steps are downloaded and returned in the `artifacts` field of the tool response |
| `EASY_MODE_MCP_MAX_ARTIFACT_SIZE` | `100000` | Maximum size of a single returned artifact, in characters for text artifacts and bytes for binary ones. Larger artifacts are truncated and flagged with `truncated: true` |
